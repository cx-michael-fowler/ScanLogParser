...
Begin {
    ...
    Function WriteErrorsToExcel {
        ...
        #Headers
        $worksheet.Range("A1") = "Line Number"
        $worksheet.Range("B1") = "DateTime"
        $worksheet.Range("C1") = "Location"
        $worksheet.Range("D1") = "Message"
        $worksheet.Range("A1:D1").Font.Bold=$True
        ...
        foreach ($error in $errors) {
            $worksheet.Range("A$i") = $error.Line
            try { $worksheet.Range("B$i") = $error.DateTime.ToString("dd/MM/yyyy HH\:mm\:ss,fff") }
